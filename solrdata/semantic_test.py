...
def solr_knn_query(endpoint, collection, embedding):
    ...
    data = {
        ...
        "fl": "id,title,short_summary,long_summary,score",
        "rows": 40,
        "wt": "json"
    }
...
def display_results(results, output_file=None):
    ...
    for doc in docs:
        result = {
            "id": doc.get("id"),
            "title": doc.get("title"),
            "short_summary": doc.get("short_summary", "N/A"),
            "long_summary": doc.get("long_summary", "N/A"),
            "score": doc.get("score")
        }
...
def main():
    ...
    query_text = "Luffy loses a battle"
